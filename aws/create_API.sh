...
for func in GET POST PUT DELETE; do 
  aws apigateway put-method --rest-api-id npfisvw5ya --resource-id ab4qgt \
    --http-method $func --authorization-type NONE

  aws apigateway put-integration --rest-api-id npfisvw5ya --resource-id ab4qgt \
  ...
  aws apigateway put-method-response --rest-api-id npfisvw5ya --resource-id ab4qgt \
    --http-method $func --status-code 200 --response-models '{"application/json": "Empty"}'

  aws apigateway put-integration-response --rest-api-id npfisvw5ya --resource-id ab4qgt \
